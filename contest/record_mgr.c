...
RC createTable(char *name, Schema *schema) 
{
    SM_FileHandle fh;
    SM_PageHandle ph;
    CHECK(createPageFile(name)); /* create new page file with filename as table name to add the table data */
    CHECK(openPageFile(name, &fh)); /* open the page file */
    ...
    CHECK(writeBlock (0, &fh, ph)); 
    CHECK(appendEmptyBlock(&fh)); /* append empty page if required */
...
RC openTable(RM_TableData *rel, char *name) 
{
    ...
    Schema *schema;
    CHECK(openPageFile(name, &fh)); /* open the page file and read the table information */
    CHECK(readBlock(0, &fh, ph));
    int pos = 0, tr = 0;
...
RC next(RM_ScanHandle *scan, Record *record) 
{
    ...
    ph = (char *) malloc(sizeof(char) * PAGE_SIZE);
    memset(ph, 0, sizeof(char) * PAGE_SIZE);
    CHECK(openPageFile(m->name, &fh));
    
    while (curp < tpage) 
   {
       	CHECK(readBlock(curp, &fh, ph)); /* read curp */
